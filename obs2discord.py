...
body = { "content":"EURE_NACHRICHT_HIER" }
...
def on_event(event):
    if event == S.OBS_FRONTEND_EVENT_STREAMING_STARTING:
        if S.obs_frontend_get_current_profile() == profilename:
...
# Zeug für Troubleshooting(Event oben kann hiermit  getauscht werden, um die Aktion mit einem Szenenwechsel auszulösen(für testzwecke)
